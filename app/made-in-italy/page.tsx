import { getStaticPageMetadata } from "@/lib/staticPageMeta";
import MadeInItalyClient from "./MadeInItalyClient";
import prisma from "@/lib/prisma";

export const revalidate = 0;

export async function generateMetadata() {
  return getStaticPageMetadata(
    "made-in-italy",
    "Made in Italy",
    "What Made in Italy means for Porcellana Nobilita — the process, provenance, and quality behind every slab."
  );
}

export default async function MadeInItalyPage() {
  let cmsData: any = null;
  try {
    const settings = await prisma.settings.findUnique({ where: { id: "global" } });
    cmsData = {
      miHeading: settings?.miHeading,
      miHeadingColor: settings?.miHeadingColor,
      miHeadingFont: settings?.miHeadingFont,
      miHeadingSize: settings?.miHeadingSize,
      miSec1Video: settings?.miSec1Video,
      miSec1Label: settings?.miSec1Label,
      miSec1LabelColor: settings?.miSec1LabelColor,
      miSec1LabelFont: settings?.miSec1LabelFont,
      miSec1LabelSize: settings?.miSec1LabelSize,
      miSec2Para1: settings?.miSec2Para1,
      miSec2Para1Color: settings?.miSec2Para1Color,
      miSec2Para1Font: settings?.miSec2Para1Font,
      miSec2Para1Size: settings?.miSec2Para1Size,
      miSec2Para2: settings?.miSec2Para2,
      miSec2Para2Color: settings?.miSec2Para2Color,
      miSec2Para2Font: settings?.miSec2Para2Font,
      miSec2Para2Size: settings?.miSec2Para2Size,
      miSec2Image: settings?.miSec2Image,
      miSec2ImageLabel: settings?.miSec2ImageLabel,
      miSec2ImageLabelColor: settings?.miSec2ImageLabelColor,
      miSec2ImageLabelFont: settings?.miSec2ImageLabelFont,
      miSec2ImageLabelSize: settings?.miSec2ImageLabelSize,
      miSec3Line1: settings?.miSec3Line1,
      miSec3Line1Color: settings?.miSec3Line1Color,
      miSec3Line1Font: settings?.miSec3Line1Font,
      miSec3Line1Size: settings?.miSec3Line1Size,
      miSec3Line2: settings?.miSec3Line2,
      miSec3Line2Color: settings?.miSec3Line2Color,
      miSec3Line2Font: settings?.miSec3Line2Font,
      miSec3Line2Size: settings?.miSec3Line2Size,
      miSec3LeftImage: settings?.miSec3LeftImage,
      miSec3LeftImageLabel: settings?.miSec3LeftImageLabel,
      miSec3LeftImageLabelColor: settings?.miSec3LeftImageLabelColor,
      miSec3LeftImageLabelFont: settings?.miSec3LeftImageLabelFont,
      miSec3LeftImageLabelSize: settings?.miSec3LeftImageLabelSize,
      miSec3LeftImageLabelShow: settings?.miSec3LeftImageLabelShow,
      miSec3RightImage: settings?.miSec3RightImage,
      miSec3RightImageLabel: settings?.miSec3RightImageLabel,
      miSec3RightImageLabelColor: settings?.miSec3RightImageLabelColor,
      miSec3RightImageLabelFont: settings?.miSec3RightImageLabelFont,
      miSec3RightImageLabelSize: settings?.miSec3RightImageLabelSize,
      miSec3BottomPara: settings?.miSec3BottomPara,
      miSec3BottomParaColor: settings?.miSec3BottomParaColor,
      miSec3BottomParaFont: settings?.miSec3BottomParaFont,
      miSec3BottomParaSize: settings?.miSec3BottomParaSize,
      miSec4BgImage: settings?.miSec4BgImage,
      miSec4BgImageMobile: settings?.miSec4BgImageMobile,
      miSec4TagImage: settings?.miSec4TagImage,
      miSec4Label: settings?.miSec4Label,
      miSec4LabelColor: settings?.miSec4LabelColor,
      miSec4LabelFont: settings?.miSec4LabelFont,
      miSec4LabelSize: settings?.miSec4LabelSize,
    };
  } catch (e) {
    cmsData = null;
  }
  return <MadeInItalyClient cmsData={cmsData} />;
}
